...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Exibir uma imagem em ASCII com cores ANSI')

    parser.add_argument('path',
                       metavar='path',
                       type=str,
                       help='o caminho para o arquivo de imagem ')
    parser.add_argument('--no-color',
                       action='store_true',
                       help='sem cores no ANSI')
    parser.add_argument('--width',
                       action='store',
                       type=int,
                       default=120,
                       help='largura de saída em caracteres (padrão 120)')
    parser.add_argument('--height-scale',
                        action='store',
                        default=.55,
                        type=float,
                        help='proporção de escala para altura (padrão 0,55')
